...
public class evaluateInfix
{
    ...
    public static int evaluate(String expression)
    {
        ...
        for (int i = 0; i < tokens.length; i++)
        {
                     ...
                     )
            {
                ...
                while (!operators.isEmpty() && hasPrecedence(tokens[i],(char) operators.peek()))
                {	values.push(applyOp(operators.pop(), values.pop(),values.pop()));}
                           ...
                           char op1, char op2)
    {
        ...
        if ((op1 == '*' || op1 == '/') && (op2 == '+' || op2 == '-'))
            return false;
        if ((op1 == '*' || op1 == '/' || op1 == '+' || op1 == '-')&& (op2 == '<' || op2 =='>' /* ||op1 == '<=' || op1 == '>='*/))
        	return false;
        else
            return true;
    }
    ...
    {
        System.out.println(evaluateInfix.evaluate("10 + (2 * 6 > 3)"));
        System.out.println(evaluateInfix.evaluate("100 * 2 + 12"));
        System.out.println(evaluateInfix.evaluate("100 * ( 2 + 12 )"));
        System.out.println(evaluateInfix.evaluate("100 * ( 2 + 12 ) / 14"));
    }
}